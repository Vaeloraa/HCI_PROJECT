...
const I18n = {
    ...
    translations: {
        en: {
            ...
            'report.assistManual': 'Manually generated: {count}',
            'report.assistStruggle': 'High-struggling triggered: {count}',
            ...
            'deepReading.startTitle': 'Clear previous records and start a new reading session',
            'deepReading.endTitle': 'End this reading session and generate a report',
            'deepReading.started.title': 'Reading started',
            'deepReading.started.sub': 'Previous records cleared — session timer running',
            'deepReading.ended.title': 'Reading ended',
            'deepReading.ended.sub': 'Session report generated for this reading period',
            'deepReading.statusActive': 'Reading {time}',
            'deepReading.statusIdle': 'No active session',
        ...
        zh: {
            ...
            'report.assistManual': '主动生成：{count} 次',
            'report.assistStruggle': '高度困难触发：{count} 次',
            ...
            'deepReading.startTitle': '清空之前记录，开始新的阅读会话',
            'deepReading.endTitle': '结束本次阅读并生成报告',
            'deepReading.started.title': '已开始阅读',
            'deepReading.started.sub': '已清空之前记录，计时开始',
            'deepReading.ended.title': '阅读已结束',
            'deepReading.ended.sub': '已生成本次阅读时段的报告',
            'deepReading.statusActive': '阅读中 {time}',
            'deepReading.statusIdle': '尚未开始',
    ...
    updateLangToggle() {
        document.querySelectorAll('.ff-lang-toggle-btn, #ff-lang-toggle .ff-segment-btn').forEach((btn) => {
            ...
            if (btn.dataset.lang === 'zh') btn.textContent = this.t('lang.zh');
            if (btn.dataset.lang === 'en') btn.textContent = this.t('lang.en');
        });
    },
    ...
    applyElement(root) {
        ...
        root.querySelectorAll('#ff-features-grid .ff-feature-label').forEach((el, index) => {
            if (featureKeys[index]) el.textContent = this.t(featureKeys[index]);
        });

        const timer = root.querySelector('#ff-timer-widget');
        if (timer) timer.title = this.t('timer.title');
        const timerStart = root.querySelector('#ff-timer-start');
        if (timerStart) timerStart.title = this.t('timer.start');
        const timerMode = root.querySelector('#ff-timer-mode');
        if (timerMode) timerMode.title = this.t('timer.mode');
        const timerReset = root.querySelector('#ff-timer-reset');
        if (timerReset) timerReset.title = this.t('timer.reset');
        const gazeToggle = root.querySelector('#btn-gaze-dot');
        if (gazeToggle) {
            gazeToggle.title = this.t('gaze.toggle');
            gazeToggle.setAttribute('aria-label', this.t('aria.gazeToggle'));
        }
    }
};

Object.assign(I18n.translations.en, {
    'runtime.file.title': 'Camera cannot start in file:// mode',
    'runtime.file.sub': 'Run npm start in the project folder, then open http://127.0.0.1:8080.',
    'runtime.unsupported.title': 'This browser does not support the camera API',
    'runtime.unsupported.sub': 'Use Chrome, Edge, or Firefox and open via localhost or HTTPS.',
    'runtime.insecure.title': 'Camera requires a secure page context',
    'runtime.insecure.sub': 'Open http://127.0.0.1:8080 or http://localhost:8080 instead of file:// or an insecure URL.',
    'aria.trackingMode': 'Tracking mode',
    'aria.language': 'Language',
    'aria.gazeToggle': 'Toggle gaze dot',
    'aria.close': 'Close',
    'goal.placeholder.words': 'e.g. 5000',
    'goal.placeholder.time': 'e.g. 30',
    'goal.minRemaining': '{min} min remaining',
    'goal.setLog': 'Goal set: {words} {time}',
    'goal.wordsUnit': '{count} words',
    'goal.minutesUnit': '{count} min',
    'summary.goalCompletion': 'Goal completion: {percent}%',
    'summary.frequentDistractions': 'Frequent distractions ({count} times). Consider enabling Deep Focus Mode and setting a countdown timer to reduce interruptions.',
    'summary.goodFocus': 'Good focus, with only {count} brief distractions.',
    'summary.excellent': 'Excellent performance. This reading session stayed highly focused.',
    'summary.longRead': 'You have read for over 30 minutes. Take a 5-10 minute break to protect your eyes.',
    'summary.title': 'Session Summary',
    'summary.readingTime': 'Reading Time',
    'summary.wordsRead': 'Words Read',
    'summary.avgWpm': 'Avg WPM',
    'summary.noInsight': 'No insight data yet.',
    'summary.endReset': 'End and Reset',
    'summary.generatedLog': 'Session summary generated',
    'summary.endedLog': 'Session ended and reset',
    'log.reset': 'System reset',
    'log.pageLoaded': 'Page loaded',
    'log.demoHint': 'Click "Start Demo Mode" to test',
    'log.faceApiAvailable': 'FaceDetector API available',
    'log.faceApiUnavailable': 'FaceDetector API unavailable; using simple detection mode',
    'log.gazeDot': 'Gaze dot: {state}',
    'log.shown': 'Shown',
    'log.hidden': 'Hidden',
    'log.cleared': 'Log cleared',
    'timer.countdownFinished': 'Countdown finished!',
    'timer.paused': 'Timer paused',
    'timer.started': 'Timer started ({mode})',
    'timer.countUp': 'count-up',
    'timer.countdown': 'countdown',
    'timer.durationLog': 'Countdown duration: {min} min',
    'timer.switchedCountdown': 'Switched to countdown ({min} min)',
    'timer.switchedCountUp': 'Switched to count-up mode',
    'timer.resetLog': 'Timer reset',
    'camera.stopped': 'Camera stopped',
    'camera.error.denied': 'Camera permission was denied - allow camera access in browser settings',
    'camera.error.notFound': 'No camera device detected - connect a camera and retry',
    'camera.error.inUse': 'The camera is being used by another app - close the app using it',
    'camera.error.constraints': 'The camera does not support the current settings - use default camera parameters',
    'camera.error.abort': 'Camera access was interrupted - retry',
    'camera.error.security': 'Security restriction: the camera API requires HTTPS or localhost',
    'camera.error.unknown': 'Unknown error ({name})',
    'api.error.failedTitle': '{api} API call failed',
    'api.error.suggestions': 'Suggestions',
    'api.error.demoTip': 'Tip: click "{button}" to simulate gaze tracking with the mouse',
    'demo.started': 'Demo mode started - simulating user behavior...',
    'demo.stopped': 'Demo stopped',
    'demo.startButton': 'Start Demo Mode',
    'demo.stopButton': 'Stop Demo',
    'scenario.prefix': 'Scenario: {name}',
    'scenario.changed': 'Scenario changed to {name}',
    'scenario.Normal Reading': 'Normal Reading',
    'scenario.Distracted - No Face': 'Distracted - No Face',
    'scenario.Recovering': 'Recovering',
    'scenario.Struggling - Long Dwell': 'Struggling - Long Dwell',
    'scenario.Recovering from Struggle': 'Recovering from Struggle',
    'scenario.Normal': 'Normal',
    'scenario.Distracted - Long': 'Distracted - Long',
    'scenario.Recovering Slowly': 'Recovering Slowly',
    'feature.detected': 'Detected',
    'feature.notDetected': 'Not detected',
    'feature.center': 'Center',
    'feature.centerCenter': 'Center-center',
    'feature.moving': 'Moving',
    'feature.idle': 'Idle',
    'feature.paused': 'Paused',
    'feature.scrolling': 'Scrolling',
    'feature.active': 'Active',
    'feature.noInteraction': 'No interaction',
    'import.large': 'Imported text is very large ({size} KB). Truncated to {limit} KB for smooth performance.',
    'strategy.subtle_overlay.name': 'Subtle Focus Overlay',
    'strategy.subtle_overlay.desc': 'Semi-transparent overlay to guide attention back',
    'strategy.floating_prompt.name': 'Floating Prompt',
    'strategy.floating_prompt.desc': 'Floating message asking to refocus',
...
Object.assign(I18n.translations.zh, {
    'app.title': 'FocusFlow - 自适应注意力阅读系统',
    'app.tagline': '自适应注意力阅读系统',
    'lang.en': 'EN',
    'lang.zh': '中文',
    'lang.current': '中文',
    ...
    'btn.startCamera': '启动摄像头',
    'btn.paragraphDebug': '分段调试',
    'btn.paragraphDebugOn': '分段调试：开',
    'btn.sessionReport': '阅读报告',

    'comprehension.title': '段落概览',
    'comprehension.meta': '第 {index} 段 - 停留 {seconds} 秒',
    'comprehension.hint': '可随时关闭；悬停段落后点击按钮可再次查看。',
    'comprehension.loading': '正在生成段落概览...',
    'comprehension.generate': '生成概览',
    'comprehension.reopen': '查看概览',
    ...
    'report.recovery': '平均恢复',
    'report.heatmapTitle': '段落注意力热力图',
    'report.heatmapHint': '柱长表示每个段落的停留时间。',
    'report.timelineTitle': '认知状态时间线',
    'report.assistTitle': '理解辅助',
    'report.assistCount': '已显示概览卡片：{count} 次',
    'report.paragraphLabel': '第 {index} 段',
    'report.noData': '数据还不够，请继续阅读一会儿。',
    'report.export': '导出 JSON',
    'report.close': '关闭',
    ...
    'timer.modeUpTitle': '正计时模式，点击切换为倒计时',
    'timer.modeDownTitle': '倒计时 {min} 分钟，点击切换为正计时',
    'timer.presetHint': '倒计时 {min} 分钟，点击修改时长',
    'timer.countUpHint': '已阅读时间',
    'timer.reset': '重置',
    'timer.timeUp.title': '时间到！',
    'timer.timeUp.sub': '本次阅读已结束，休息一下再继续吧。',

    'gaze.toggle': '显示或隐藏视线位置指示',
    ...
    'camera.previewBadge': '4:3 预览',
    'camera.emptyTitle': '摄像头预览',
    'camera.emptySub': '切换到眼动模式后，这里会显示 WebGazer 人脸网格。',
    'camera.inputSource': '输入来源',
    'camera.trackingStatus': '追踪状态',
    'camera.visualFeedback': '视觉反馈',
    'camera.cursorMeaning': '光标含义',
    'camera.greenMesh': '绿色网格',
    'camera.realGaze': '真实视线',
    'camera.gate.title': '启用眼动追踪',
    'camera.gate.prompt': 'FocusFlow 需要摄像头权限来进行 WebGazer 眼动追踪。点击下方按钮后，浏览器会请求授权。',
    'camera.gate.denied': '摄像头权限被阻止。请在浏览器设置中允许摄像头访问，然后重试。',
    'camera.gate.file': '请通过本地服务器打开页面，不要使用 file://。运行 npm start 后访问 http://127.0.0.1:8080。',
    'camera.gate.unsupported': '当前浏览器不支持摄像头 API，请使用 Chrome、Edge 或 Firefox。',
    'camera.gate.enable': '启用摄像头并校准',
    'camera.gate.retry': '重试摄像头',
    'camera.gate.working': '正在启动摄像头...',
    'camera.suggest.allowAddressBar': '点击地址栏中的锁或摄像头图标，允许摄像头访问',
    'camera.suggest.resetPermission': '如果之前拒绝过权限，请在浏览器站点设置中重置摄像头权限',
    'camera.suggest.retryButton': '允许权限后，点击“启用摄像头”或“重试摄像头”',
    'camera.suggest.useLocalhost': '请访问 http://127.0.0.1:8080 或 http://localhost:8080，不要使用 file://',
    'camera.suggest.useChromeEdge': '建议使用最新版 Chrome、Edge 或 Firefox',
    'camera.suggest.closeOtherApps': '关闭可能占用摄像头的其他应用',
    'camera.suggest.connectCamera': '请连接摄像头，并确认系统设置中未禁用',
    ...
    'state.Normal': '正常',
    'state.Distracted': '分心',
    'state.Struggling': '理解困难',
    'state.Recovering': '恢复中',
    'state.OffScreen': '离开屏幕',
    'state.desc.Normal': '正在专注阅读',
    'state.desc.Distracted': '注意力已偏离',
    'state.desc.Struggling': '当前内容可能较难理解',
    ...
    'goal.hint': '设置目标后，系统会实时追踪进度，并在结束时生成总结。',
    'goal.unlimited': '不限',
    'goal.placeholder.words': '例如 5000',
    'goal.placeholder.time': '例如 30',
    'goal.minRemaining': '剩余 {min} 分钟',
    'goal.setLog': '已设置目标：{words} {time}',
    'goal.wordsUnit': '{count} 字',
    'goal.minutesUnit': '{count} 分钟',
    ...
    'tracking.gaze': '眼动',
    'tracking.mouseTitle': '鼠标追踪（无需摄像头）',
    'tracking.gazeTitle': '眼动追踪（需要摄像头）',

    'welcome.title': '欢迎使用 FocusFlow！',
    'welcome.sub': '默认使用鼠标追踪。可在顶部切换专注模式、追踪方式和语言。',

    'calibration.title': 'WebGazer 校准',
    'calibration.instruction': '注视高亮点，然后点击或按空格键。',
    'calibration.point': '校准点 {current} / {total}',
    'calibration.waiting': '等待中...',
    'calibration.skip': '跳过校准',
    'calibration.complete.title': '校准完成！',
    ...
    'calibration.skipped.sub': '已跳过校准，视线将跟随鼠标光标。',

    'prompt.stillThere.title': '还在吗？',
    'prompt.stillThere.sub': '看起来你暂时离开了，准备好后继续阅读吧。',
    'prompt.reminder1.text': '准备好了就回来吧。',
    'prompt.reminder1.sub': '阅读内容还在这里等你。',
    'prompt.reminder2.text': '一次读一句就好。',
    'prompt.reminder2.sub': '不用一下子追上所有内容。',
    'prompt.reminder3.text': '休息一下也没关系。',
    'prompt.reminder3.sub': '方便时再回来就好。',
    'prompt.reminder4.text': '你刚才读得很好。',
    'prompt.reminder4.sub': '从停下的地方继续吧。',
    'prompt.wakeup.title': '该重新专注了！',
    'prompt.wakeup.sub': '你已经离开一段时间了，从上次停下的地方继续吧。',
    'prompt.struggling.title': '这一段可以慢慢读。',
    'prompt.struggling.sub': '不用着急，理解比速度更重要。',

    'api.error.title': '{api} 需要处理',
    'api.error.sub': '{summary}。请检查摄像头权限后重试。',

    'debug.paragraph.title': '分段调试',
    'debug.paragraph.toggleTitle': '显示或隐藏段落边界',
    'debug.paragraph.track': '追踪段落（视线/NLP）',
    'debug.paragraph.heading': '小节标题（不追踪）',
    'debug.paragraph.titleBlock': '标题 / 副标题（不追踪）',
    'debug.paragraph.stats': '共 {total} 块 - {tracked} 个追踪段落 - {headings} 个标题 - {titles} 个标题块',
    'debug.paragraph.badgeTrack': '第 {index} 段 - {chars} 字符 - {words} 词',
    'debug.paragraph.badgeHeading': '标题 - {chars} 字符',
    'debug.paragraph.typeTitle': '主标题',
    'debug.paragraph.typeSubtitle': '副标题',
    'debug.panel': '调试面板',
    'debug.gaze': '视线：',
    ...
    'insight.break': '注意力似乎有些分散，要不要短暂休息一下？',
    'insight.paceSlow': '你在重复阅读一些段落，稍微放慢可能更有助于理解。',
    'insight.encourage': '专注状态很好！你正在稳定阅读。',
    'insight.paceInfo': '你读得比较从容，这对理解内容很有帮助。',
    'insight.paceFast': '你读得很快，请确认自己真正吸收了内容。',
    ...
    'strategy.waiting': '等待状态变化...',
    'strategy.subtle_overlay.name': '轻微专注遮罩',
    'strategy.subtle_overlay.desc': '用半透明遮罩帮助注意力回到阅读内容',
    'strategy.floating_prompt.name': '悬浮提示',
    'strategy.floating_prompt.desc': '显示一条悬浮消息提醒重新专注',
    'strategy.sound_alert.name': '声音提醒',
    'strategy.sound_alert.desc': '播放轻柔提示音唤回注意力',
    'strategy.keyword_highlight.name': '关键词高亮',
    'strategy.keyword_highlight.desc': '高亮内容中的关键术语',
    'strategy.summary_panel.name': '摘要面板',
    'strategy.summary_panel.desc': '显示当前段落的简短概览',
    'strategy.simplification.name': '内容简化',
    'strategy.simplification.desc': '显示复杂内容的简化版本',
    'strategy.progress_indicator.name': '进度提示',
    'strategy.progress_indicator.desc': '显示阅读进度以鼓励继续',
    'strategy.positive_feedback.name': '正向反馈',
    'strategy.positive_feedback.desc': '在恢复专注后显示鼓励信息',
    'escalation.low': '低',
    'escalation.medium': '中',
    'escalation.high': '高',

    'runtime.file.title': 'file:// 模式下无法启动摄像头',
    'runtime.file.sub': '请在项目目录运行 npm start，然后打开 http://127.0.0.1:8080。',
    'runtime.unsupported.title': '当前浏览器不支持摄像头 API',
    'runtime.unsupported.sub': '请使用 Chrome、Edge 或 Firefox，并通过 localhost 或 HTTPS 打开。',
    'runtime.insecure.title': '摄像头需要安全页面环境',
    'runtime.insecure.sub': '请打开 http://127.0.0.1:8080 或 http://localhost:8080，不要使用 file:// 或不安全地址。',
    'aria.trackingMode': '追踪模式',
    'aria.language': '语言',
    'aria.gazeToggle': '切换视线点',
    'aria.close': '关闭',

    'summary.goalCompletion': '目标完成度：{percent}%',
    'summary.frequentDistractions': '分心较频繁（{count} 次）。建议启用深度专注模式，并设置倒计时来减少打断。',
    'summary.goodFocus': '专注表现不错，只出现了 {count} 次短暂分心。',
    'summary.excellent': '表现很棒，本次阅读保持了很高的专注度。',
    'summary.longRead': '你已经阅读超过 30 分钟了，建议休息 5-10 分钟保护眼睛。',
    'summary.title': '会话总结',
    'summary.readingTime': '阅读时间',
    'summary.wordsRead': '已读字数',
    'summary.avgWpm': '平均速度',
    'summary.noInsight': '暂无总结数据。',
    'summary.endReset': '结束并重置',
    'summary.generatedLog': '已生成会话总结',
    'summary.endedLog': '会话已结束并重置',
    'log.reset': '系统已重置',
    'log.pageLoaded': '页面已加载',
    'log.demoHint': '点击“演示模式”开始测试',
    'log.faceApiAvailable': 'FaceDetector API 可用',
    'log.faceApiUnavailable': 'FaceDetector API 不可用，已使用简化检测模式',
    'log.gazeDot': '视线点：{state}',
    'log.shown': '显示',
    'log.hidden': '隐藏',
    'log.cleared': '日志已清空',
    'timer.countdownFinished': '倒计时结束！',
    'timer.paused': '计时已暂停',
    'timer.started': '计时已开始（{mode}）',
    'timer.countUp': '正计时',
    'timer.countdown': '倒计时',
    'timer.durationLog': '倒计时时长：{min} 分钟',
    'timer.switchedCountdown': '已切换为倒计时（{min} 分钟）',
    'timer.switchedCountUp': '已切换为正计时',
    'timer.resetLog': '计时已重置',
    'camera.stopped': '摄像头已停止',
    'camera.error.denied': '摄像头权限被拒绝，请在浏览器设置中允许访问',
    'camera.error.notFound': '未检测到摄像头，请连接摄像头后重试',
    'camera.error.inUse': '摄像头正被其他应用占用，请关闭占用它的应用',
    'camera.error.constraints': '摄像头不支持当前设置，请使用默认摄像头参数',
    'camera.error.abort': '摄像头访问被中断，请重试',
    'camera.error.security': '安全限制：摄像头 API 需要 HTTPS 或 localhost',
    'camera.error.unknown': '未知错误（{name}）',
    'api.error.failedTitle': '{api} API 调用失败',
    'api.error.suggestions': '建议',
    'api.error.demoTip': '提示：点击“{button}”可用鼠标模拟视线追踪',
    'demo.started': '演示模式已启动，正在模拟用户行为...',
    'demo.stopped': '演示已停止',
    'demo.startButton': '启动演示模式',
    'demo.stopButton': '停止演示',
    'scenario.prefix': '场景：{name}',
    'scenario.changed': '场景已切换到：{name}',
    'scenario.Normal Reading': '正常阅读',
    'scenario.Distracted - No Face': '分心：未检测到人脸',
    'scenario.Recovering': '恢复中',
    'scenario.Struggling - Long Dwell': '理解困难：长时间停留',
    'scenario.Recovering from Struggle': '从困难中恢复',
    'scenario.Normal': '正常',
    'scenario.Distracted - Long': '长时间分心',
    'scenario.Recovering Slowly': '缓慢恢复',
    'feature.detected': '已检测',
    'feature.notDetected': '未检测到',
    'feature.center': '居中',
    'feature.centerCenter': '中心区域',
    'feature.moving': '移动中',
    'feature.idle': '空闲',
    'feature.paused': '暂停',
    'feature.scrolling': '滚动中',
    'feature.active': '活跃',
    'feature.noInteraction': '无交互',
    'import.large': '导入文本较大（{size} KB），已截断为 {limit} KB 以保持流畅。'
});

I18n.translateStrategy = function(strategy) {
    if (!strategy) return { name: this.t('strategy.none'), description: this.t('strategy.noneDesc') };
    const id = strategy.id || 'none';
    return {
        name: this.t(`strategy.${id}.name`) !== `strategy.${id}.name` ? this.t(`strategy.${id}.name`) : (strategy.name || this.t('strategy.none')),
        description: this.t(`strategy.${id}.desc`) !== `strategy.${id}.desc` ? this.t(`strategy.${id}.desc`) : (strategy.description || this.t('strategy.noneDesc'))
    };
};
